...
- ✅ **New Backend**: Migrated from Genspark Tables API to Supabase
- ✅ **PostgreSQL Power**: Real database with proper indexing and RLS
- ✅ **Row Level Security**: Data protection ready for the future
- ✅ **Faster Queries**: Indexed lookups on username, created_at
- ✅ **Service Worker v11**: Fresh cache for the new architecture
- ✅ **Daily Deposit Fixed**: REQUIRED VOCAB now shows properly with offline fallbacks

**Supabase Project:**
- URL: `https://tilpgwoyyervbgdlucap.supabase.co`
- Tables: `users`, `bars`, `songs`, `prompts_*`, `community_submissions`

**Database Schema** (see `supabase-schema.sql`):
```sql
-- Core Tables
users          -- User accounts
bars           -- Lyrics/ideas with tags, audio, images  
songs          -- Full tracks with block editor

-- Daily Deposit Vault
prompts_feelings, prompts_settings, prompts_objects, prompts_smells, prompts_vocab

-- Community
community_submissions  -- The Syndicate feed
```
...
- ✅ **Version Indicator**: Added "v8" to the bottom status bar so you know you're on the latest drop.
- ✅ **Deployment Ready**: Files are optimized for static hosting (Netlify/GitHub Pages).
