...
const api = {
    async get(table, params = {}) {
        ...
        const url = `${API_BASE}/${table}${query ? '?' + query : ''}`;
        
        if (window.DEBUG_API) console.log(`🔵 API GET: ${url}`);
        
        try {
            const res = await fetch(url);
            if (!res.ok) {
                console.error(`❌ API GET /${table} failed:`, res.status, res.statusText);
                console.error(`📍 Full URL attempted: ${window.location.origin}/${url}`);
                if (res.status === 500) {
                    console.error('🔧 Database may be misconfigured. Contact support if this persists.');
                }
                if (res.status === 404) {
                    console.error('🔧 Table not found - ensure schema exists in Genspark.');
                }
                return { data: [], error: true, status: res.status };
            }
            const data = await res.json();
            if (window.DEBUG_API) console.log(`✅ API GET /${table}:`, data);
            return data;
        } catch (err) {
            console.error(`❌ API GET /${table} network error:`, err);
            console.error(`📍 This usually means CORS issue or wrong domain`);
            return { data: [], error: true, status: 0 };
        }
    },
    async create(table, data) {
        const url = `${API_BASE}/${table}`;
        if (window.DEBUG_API) console.log(`🟢 API POST: ${url}`, data);
        ...
        try {
            ...
            if (!res.ok) {
                console.error(`❌ API POST /${table} failed:`, res.status);
                const errorText = await res.text().catch(() => 'No error body');
                console.error(`📝 Error response:`, errorText);
                throw new Error(`Create failed: ${res.status}`);
            }
            const result = await res.json();
            if (window.DEBUG_API) console.log(`✅ API POST /${table} success:`, result);
            return result;
        } catch (err) {
    ...
    async update(table, id, data) {
        ...
        if (!res.ok) {
            console.error(`❌ API PUT /${table}/${id} failed:`, res.status);
    ...
    async patch(table, id, data) {
        ...
        if (!res.ok) {
            console.error(`❌ API PATCH /${table}/${id} failed:`, res.status);
    ...
    async delete(table, id) {
        ...
        if (!res.ok) {
            console.error(`❌ API DELETE /${table}/${id} failed:`, res.status);
