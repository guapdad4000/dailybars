...
window.BeatAnalyzer = function () {
  ...
  async function analyzeAudio(file, options = {}) {
    ...
    try {
      ...
      const analyses = [];
      if (detectBpm) {
        analyses.push(detectBPM(channelData, sampleRate).then(r => {
          results.bpm = r.bpm;
          results.bpmConfidence = r.confidence;
        }));
      }
      if (detectKey) {
        analyses.push(detectMusicalKey(channelData, sampleRate, audioContext).then(r => {
          results.key = r.key;
          results.keyConfidence = r.confidence;
        }));
      }
      if (detectEnergy) {
        analyses.push(analyzeEnergy(channelData).then(r => {
          results.energy = r.energy;
          results.danceability = r.danceability;
        }));
      }
      if (generateWaveform) {
        analyses.push(generateWaveformData(channelData).then(r => {
          results.waveform = r;
        }));
      }
      if (extractMetadata) {
        analyses.push(extractID3Metadata(file).then(r => {
          results.metadata = r;
        }));
      }
  ...
  function computeSpectrum(samples, fftSize) {
    ...
    // Apply Hann window
    const windowed = samples.map((s, i) => s * 0.5 * (1 - Math.cos(2 * Math.PI * i / samples.length)));
      ...
      try {
        ...
        reader.onload = function (e) {
          ...
          if (buffer.byteLength >= 128) {
            ...
            const tag = String.fromCharCode(view.getUint8(tagOffset), view.getUint8(tagOffset + 1), view.getUint8(tagOffset + 2));
            if (tag === 'TAG') {
